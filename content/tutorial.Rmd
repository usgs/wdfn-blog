...
```{r fig.height=3.5, title="Simple plot of flow over time."}
library(ggplot2)
...
```{r fig.height=3.5, fig.cap="Flow versus time with pretty labels."}
...
```{r echo=FALSE}
knitr::include_graphics("static/dataRetrieval/waterservices.png")
...
```{r fig.height=4, fig.cap="Map of Arizona with phosphorus."}
library(ggplot2)
library(ggsn)
library(sf)
library(dplyr)
...
Then map it:
```{r echo=FALSE, fig.cap="Wisconsin phosphorous map."}
...
```{r echo=FALSE, fig.cap = "Wisconsin phosphorous sites via WQP."}
