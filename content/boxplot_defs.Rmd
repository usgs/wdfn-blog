...
boxplot_framework <- function(upper_limit, x, 
  ...
  n_fun <- function(x){
    return(data.frame(y = upper_limit*0.95, label = length(x)))
  }
  
  return(list(stat_boxplot(geom ='errorbar', width = width),
              geom_boxplot(width = 0.6, fill = fill),
              stat_summary(fun.data = n_fun, geom = "text"),
              expand_limits(y = 0),
              scale_y_continuous(sec.axis = dup_axis(label = NULL, 
                                                     name = NULL),
                                 expand = expand_scale(mult = c(0, 0)),
                                 breaks = pretty(c(0,x, upper_limit), n = 5), 
...
```{r chlorideWithLegend, fig.cap="Chloride by month styled.", fig.alt="Chloride by month, styled."}
library(cowplot)
...
```{r starWars, fig.cap="Star wars character's mass distribution by planet.", fig.alt="Star wars character's mass distribution by planet.", warning=FALSE}
