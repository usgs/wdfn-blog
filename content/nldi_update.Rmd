...
The relationship between `featureSources` that can be accessed in the NLDI and NHDPlusV2 catchments is important in understanding how the new catchment characteristics functionality works.
...
> NOTE: This post was rendered from "R Markdown" into what you are reading using a Docker-based R / Python environment ([hydrogeoenv](https://github.com/dblodgett-usgs/hydrogeoenv)) that contains dependencies for both these client applications.
...
Then, we use [WaterData](https://labs.waterdata.usgs.gov/geoserver/index.html) GeoServer, which has the same version of the NHD as the NLDI, to get all the NHDPlus attributes of the these ComIDs.
...
Next, we should sort the ComIDs topologically to get ready for later accumulation functions.

```{python}
flw = nhd.prepare_nhdplus(flw, 0, 0, purge_non_dendritic=False)
```

The available characteristic IDs for any of the three characteristic types (`local`, `tot`, `div`) can be found using `get_validchars` method of the `NLDI` class. For example, let's take a look at the `local` characteristic type:
...
Note that for a large number of ComIDs it's faster to get the whole database (all ComIDs) for the characteristic type
