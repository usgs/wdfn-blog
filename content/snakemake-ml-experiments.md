...
- For each station we needed to test the effect of a multi-task scaling factor to see how beneficial multi-tasking was; the larger the value of the multi-task scaling factor, the more the model focused on multi-tasking (predicting water temperature as well as streamflow) - the smaller the value, the more the model focused on just getting the primary task right (predicting streamflow). For the experiment we tried out 9 different values of the multi-task scaling factor (ranging from 0 to 320).
...
For example, you can import modules like Pandas or define and run a custom function just like in a .py file.
...
With these wildcards, Snakemake could use the output file path to get the correct values to pass to the `calculate_prediction` function.

For example, if we asked Snakemake to produce `"expA/site_id_01022500/factor_10/rand_seed_4/validation_metrics.csv"`, it would make the following function call in the `run` step: 
...
### `expand`ing
While wildcards are placeholders that make rules applicable to a single combination of inputs, the Snakemake `expand` function makes it easy to apply to a rule (or set of rules) to all input combinations.
...
The `expand` function makes all possible combinations of these three variables (i.e., 27,270) and runs the associated branch for each one.
...
This is just representing a small subset (8 model runs).
The actual DAG, with all 27,270 model runs would be 3400 times wider!
...
We only had to modify slightly modify the Snakemake execution command slightly.
The portability from a local compute environment to the HPC was a very nice feature 
...
With Snakemake running cluster our large experiments were executed in a less than a couple of hours.
