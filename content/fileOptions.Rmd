...
get_seconds <- function(timing, format){
  ...
  if(as.character(attr(summary(timing), "unit")) != "seconds"){
    ...
    if(unit == "milliseconds"){
      ...
      read_write <- data.frame(
        format = format,
        read = summary(timing)$mean[2]/1e9,
        write = summary(timing)$mean[1]/1e9,
        filter = summary(timing)$mean[3]/1e9,
        summarize = summary(timing)$mean[4]/1e9,
        stringsAsFactors = FALSE  )    
    } else if (unit == "microseconds"){
      read_write <- data.frame(
        format = format,
        read = summary(timing)$mean[2]/1000000,
        write = summary(timing)$mean[1]/1000000,
        filter = summary(timing)$mean[3]/1000000,
        summarize = summary(timing)$mean[4]/1000000,
        stringsAsFactors = FALSE  )      
    }
  } else {
    read_write <- data.frame(
...
read_write_rds <- get_seconds(timing_rds,"rds")

```

```{r rds_no_timing_print, echo=FALSE, message=FALSE, results="asis"}
...
knitr::kable(read_write_rds, digits = 1, 
...
Also, I am keeping this analysis as a "data.frame" (rather than "data.table") because it is the system our group has decided to stick with. Thanks to [Michael Chirico](https://twitter.com/michael_chirico) for pointing out that is more effienct to convert to a data.frame at the end of the query instead of in the initial `fread` argument.
...
Even though we are a `dplyr`-centric group, it's important to consider a switch to `data.table`-centric commands. [Michael Chirico](https://twitter.com/michael_chirico) was kind enough to provide that workflow:

```{r fread_dt, eval=FALSE}
library(data.table)
library(fasttime)
file_name <- "test.csv"
# Write:
...
fread_df <- fread(file_name, 
                  na.strings = "") 
fread_df[ , requested_date := fastPOSIXct(requested_date, tz = "America/New_York")]

# Read and Filter:
read_filter_fread <- fread(file_name, 
                           na.strings = "",
                           select = c("bytes","requested_date","parametercds"))

read_filter_fread <- read_filter_fread[bytes > min_bytes & grepl(param_cd, parametercds, fixed = TRUE)]
read_filter_fread[ , requested_date := fastPOSIXct(requested_date, tz = "America/New_York")]

# Read and Group and Summarize:
read_group_summary_fread <- fread(file_name,
                                  na.strings = "",
                                  select = c("bytes","requested_date","service",group_col))
read_group_summary_fread[ , requested_date := fastPOSIXct(requested_date, tz = "America/New_York")]

read_group_summary_fread = read_group_summary_fread[service == ..service & 
                             !is.na(group_col) & 
                             requested_date > as.POSIXct("2016-10-02 00:00:00")]

read_group_summary_fread[ , requested_date = as.Date(requested_date)]
read_group_summary_fread[ , .(MB = sum(as.numeric(bytes), na.rm = TRUE)/1e6, by = c(group_col, 'requested_date'))]

```

```{r fread_output_dt, echo=FALSE, cache=TRUE}
library(data.table)
library(fasttime)
file_name <- "test.csv"
invisible(gc())
...
read_filter_fread <- read_filter_fread[bytes > min_bytes & grepl(param_cd, parametercds, fixed = TRUE)]
read_filter_fread[ , requested_date := fastPOSIXct(requested_date, tz = "America/New_York")]
  read_filter_fread <- setDF(read_filter_fread)
  },
  summarize = {
read_group_summary_fread <- fread(file_name,
                                  na.strings = "",
...
# 
# read_group_summary_fread[ , requested_date = as.Date(requested_date)]
# read_group_summary_fread[ , .(MB = sum(as.numeric(bytes), na.rm = TRUE)/1e6, 
#                               by = c(group_col, 'requested_date'))]
read_group_summary_fread <- setDF(read_group_summary_fread)
  },
  times = times
)

read_write_fread_dt <- get_seconds(timing,"fread using dt")

```


```{r fread_dt_print, echo=FALSE}
knitr::kable(read_write_fread_dt, digits = 1, 
             format = "html", escape = FALSE,align = "c",
             col.names = table_col_names) %>%
  kable_styling(c("striped", "bordered")) 
```
...
#Gather up cached stuff:
read_write <- bind_rows(read_write_rds, 
                        read_write_rds_comp, 
                        read_write_readr,
                        read_write_readr_comp,
                        read_write_fread,
                        read_write_fread_dt,
                        read_write_feather,
                        read_write_fst,
...
tested <- c("rds","rds compressed",
            "csv (readr)","csv.gz (readr)",
            "csv (fread)","csv (full data.table)",
            "feather",
            "fst","fst compressed",
            "sqlite","monetDB"
            )
file_name <- setNames(c("test.rds","test_compressed.rds",
                        ...
                        "test.csv","test.csv","test.feather",
...
read_write <- read_write %>%
  mutate(filter = color_tile("lightgreen","white")(round(filter, digits = 0)),
         ...
         method = ifelse(format %in% c("readr","readr compression","fread",
                                       "feather","fst","fst compression","vroom"),
                         "limited partial",
                         ifelse(format %in% c("MonetDB","sqlite"), "partial", "bulk")))%>%
  select(format, method, read, filter, summarize, write, file_size)

read_write$format <- tested
...
Note that `sqlite` and `MonetDB` are the only formats here that allow careful filtering and calculate summaries without loading the whole data set (classified as a "partial" read method). So if our "pretty big data" gets "really big", those will formats will rise to the top. If you can read in all the rows without crashing R, `readr`, `fread`, `feather`, and `fst` are fast!
