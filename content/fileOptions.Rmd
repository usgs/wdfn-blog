...
timing <- microbenchmark(
  ...
  filter = {
    partial_data_rdr <- read_csv(file_name, progress = FALSE) %>%
      filter(bytes > !!min_bytes,
             grepl(!!param_cd, parametercds)) %>%
      select(bytes, requested_date, parametercds)
  ...
  summarize = {
    group_summary_rdr <- read_csv(file_name, progress = FALSE) %>%
      filter(service == !!service, 
             !is.na(!!sym(group_col)),
             requested_date > as.POSIXct("2016-10-02 00:00:00")) %>%
      mutate(requested_date = as.Date(requested_date)) %>%
...
read_write <- read_write %>%
  mutate(filter = color_tile("lightgreen","white")(round(filter, digits = 0)),
         ...
         method = ifelse(format %in% c("fread","feather","fst","fst compression","vroom"),
                         "limited partial",
