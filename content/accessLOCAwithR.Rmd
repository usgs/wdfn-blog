...
![](static/accessLOCAwithR/area_of_interest.png) ![](static/accessLOCAwithR/point_of_interest.png)
...
plot_data<-gather_(data = data.frame(dates=plot_dates, rcp45=test_var, rcp85=test_var2),
                   key_col = "Pathway", value_col = "data", gather_cols = c("rcp45","rcp85"))
ggplot(plot_data,aes(x=dates,y=data,colour=Pathway,group=Pathway)) + geom_point(alpha=1/15) +
...
```{r geoknife_setup, echo=TRUE}
suppressMessages(library(geoknife))
stencil <- webgeom()
...
while(ran_one){
  ...
  for(var_to_get in varList) {
    ...
    if(!file.exists(paste0('~/temp/loca_demo/',var_to_get,'.csv'))) {
      variables(fabric) <- var_to_get
      job <- try(geoknife(stencil, fabric, wait=TRUE))
      try(download(job,paste0('~/temp/loca_demo/',var_to_get,'.csv')))
      ran_one<-TRUE
    }
  }
}

loca_data_platte<-data.frame(dates=parseTimeseries(paste0('~/temp/loca_demo/',varList[1],'.csv'),delim=',')$DateTime)
for(var_to_parse in varList) {
  loca_data_platte[var_to_parse] <- parseTimeseries(paste0('~/temp/loca_demo/',var_to_parse,'.csv'), delim=',')[2]
}
...
```{r climates_summary, echo=FALSE}
suppressMessages(library(climates))
suppressMessages(library(PCICt))
...
for(scenario in scenarios) {
  loca_deriv_platte[[scenario]]<-list()
  for(thresh in names(thresholds)) { # Set up the empty output data structure.
    loca_deriv_platte[[scenario]][[thresh]]<-data.frame(year=c(as.numeric(years[1]):(as.numeric(years[length(years)])-1)))
    loca_deriv_platte[[scenario]][[thresh]][gcms]<-NA
  }
  ...
  for(year in 1:length(loca_deriv_platte[[scenario]][[names(thresholds)[1]]]$year)) {
    subRows<-which(years %in% loca_deriv_platte[[scenario]][[names(thresholds)[1]]]$year[year])
    ...
    for(thresh in 1:length(names(thresholds))) {
      loca_deriv_platte[[scenario]][[names(thresholds)[thresh]]][gcms][year,] <- indices[,thresh]
    }
  }
}
```
...
for(thresh in names(plot_setup)) {
  for(scenario in scenarios) {
    plot_data<-gather_(loca_deriv_platte[[scenario]][[thresh]],'gcm','data',
                       names(loca_deriv_platte[[scenario]][[thresh]])[2:length(loca_deriv_platte[[scenario]][[thresh]])])
    plot_setup[[thresh]][[paste0("plotAll",scenario)]] <- ggplot(plot_data,aes(x=year,y=data,colour=gcm,group=gcm)) + 
      plot_setup[[thresh]]$plottype +  ylab(plot_setup[[thresh]]$yaxis) + xlab('Year') + ggtitle(scenario)
    
    plot_data<-data.frame(mean=apply(loca_deriv_platte[[scenario]][[thresh]][names(loca_deriv_platte[[scenario]][[thresh]])[2:length(loca_deriv_platte[[scenario]][[thresh]])]],1,mean))
    plot_data['max']<-apply(loca_deriv_platte[[scenario]][[thresh]][names(loca_deriv_platte[[scenario]][[thresh]])[2:length(loca_deriv_platte[[scenario]][[thresh]])]],1,max)
    plot_data['min']<-apply(loca_deriv_platte[[scenario]][[thresh]][names(loca_deriv_platte[[scenario]][[thresh]])[2:length(loca_deriv_platte[[scenario]][[thresh]])]],1,min)
    plot_data['year']<-loca_deriv_platte[[scenario]][[thresh]]$year
    plot_data<-gather_(plot_data,'stat','data',c('mean','max','min'))
    plot_setup[[thresh]][[paste0("plotStats",scenario)]] <- ggplot(plot_data,aes(x=year,y=data,colour=stat,group=stat)) + plot_setup[[thresh]]$plottype +
...
grid_arrange_shared_legend(plot_setup[[thresh]]$plotAllrcp45, plot_setup[[thresh]]$plotAllrcp85,ncol=2,top=plot_setup[[thresh]]$title,legend.text=element_text(size = 6))
  }
```