...
```{r get_polygon_and_map, echo=TRUE, message=FALSE}
library(jsonlite)
library(leaflet)
...
```{r plot_the_data, echo=TRUE, eval=FALSE, message=FALSE}
library(chron)
library(tidyr)
library(ggplot2)
...
```{r grid_arrange_function, echo=TRUE, message=FALSE}
library(tidyr)
library(ggplot2)
...
```{r plotIt, echo=TRUE, warning=FALSE, message=FALSE}
